...
export function dashboardView() {
  ...
  return `
    <section class="page-heading" data-layout-area="page-header"><div><h1>Supply Dashboard</h1><p>Current stock, movements, vendors, and item value.</p></div></section>
    <section class="grid kpis" data-layout-area="kpi-summary">
      <article class="card" data-layout-area="kpi-card"><div class="kpi-label">Supply Items</div><div class="kpi-value">${number.format(state.supply.length)}</div></article>
      <article class="card" data-layout-area="kpi-card"><div class="kpi-label">Supply In</div><div class="kpi-value">${number.format(inTotal)}</div></article>
      <article class="card" data-layout-area="kpi-card"><div class="kpi-label">Supply Out</div><div class="kpi-value">${number.format(outTotal)}</div></article>
      <article class="card" data-layout-area="kpi-card"><div class="kpi-label">Stock Value</div><div class="kpi-value">${money.format(calculateStockValue())}</div></article>
    </section>
    <section class="card" data-layout-area="primary-content">
      <h2 data-layout-area="card-header">Current Stock</h2>
      <div data-layout-area="card-body">${supplyTable(false)}</div>
    </section>`;
}
...
export function movementView(direction) {
  ...
  return `
    <section class="page-heading" data-layout-area="page-header"><div><h1>Supply ${isIn ? 'In' : 'Out'}</h1><p>${isIn ? 'Receive stock from a vendor.' : 'Issue stock for use or transfer.'}</p></div></section>
    <section class="grid two">
      <form class="card" id="movement-form" data-direction="${direction}" data-layout-area="primary-content">
        <h2 data-layout-area="card-header">New ${isIn ? 'Receipt' : 'Issue'}</h2>
        <div data-layout-area="form-fields">
        ...
        <div data-layout-area="actions"><button class="btn btn-primary" type="submit">Save Supply ${isIn ? 'In' : 'Out'}</button></div>
      </form>
      <article class="card" data-layout-area="secondary-panel"><h2 data-layout-area="card-header">Recent ${isIn ? 'Receipts' : 'Issues'}</h2><div data-layout-area="card-body">${movementTable(direction)}</div></article>
    </section>`;
}

export function vendorsView() {
  return `
    <section class="page-heading" data-layout-area="page-header"><div><h1>Vendors</h1><p>Maintain supplier contact details separately from inventory items.</p></div></section>
    <section class="grid two">
      <form class="card" id="vendor-form" data-layout-area="primary-content">
        <h2 data-layout-area="card-header">${state.editingVendorId ? 'Edit Vendor' : 'Add Vendor'}</h2>
        <div data-layout-area="form-fields">
        ...
        <div class="toolbar" data-layout-area="actions"><button class="btn btn-primary" type="submit">Save Vendor</button><button class="btn btn-secondary" type="reset">Clear</button></div>
      </form>
      <article class="card" data-layout-area="secondary-panel"><h2 data-layout-area="card-header">Vendor Directory</h2><div data-layout-area="card-body">${vendorTable()}</div></article>
    </section>`;
}

export function adminView() {
  return `
    <section class="page-heading" data-layout-area="page-header"><div><h1>Admin</h1><p>Manage supply master data and database setup.</p></div></section>
    <div class="notice" data-layout-area="supporting-content">The current <strong>supply</strong> table remains the item master. Stock quantities come from the separate movement table, so editing CSS or page layout cannot change inventory mathematics.</div>
    <section class="grid two">
      <form class="card" id="supply-form" data-layout-area="primary-content">
        <h2 data-layout-area="card-header">${state.editingSupplyId ? 'Edit Supply Item' : 'Add Supply Item'}</h2>
        <div class="form-grid" data-layout-area="form-fields">
          <div class="field"><label>Name</label><input class="input" name="Name" required></div>
        ...
        <div class="toolbar" data-layout-area="actions"><button class="btn btn-primary" type="submit">Save Item</button><button class="btn btn-secondary" type="reset">Clear</button></div>
      </form>
      <article class="card" data-layout-area="secondary-panel"><h2 data-layout-area="card-header">System Structure</h2><div data-layout-area="card-body"><p>Design: <code>css/tokens.css</code> and <code>css/app.css</code></p><p>Database: <code>js/db.js</code></p><p>Calculations: <code>js/state.js</code></p><p>Pages: <code>js/views.js</code></p><p>Events: <code>js/app.js</code></p></div></article>
    </section>
    <section class="card section-gap" data-layout-area="primary-content"><h2 data-layout-area="card-header">Supply Master</h2><div data-layout-area="card-body">${supplyTable(true)}</div></section>`;
}

function supplyTable(showActions) {
  if (!state.supply.length) return '<div class="empty">No supply items found.</div>';
  return `<div class="table-wrap" data-layout-area="data-table"><table><thead><tr><th>Name</th><th>Category</th><th>Unit</th><th>Rate</th><th>Stock</th><th>Value</th>${showActions ? '<th>Actions</th>' : ''}</tr></thead><tbody>${state.supply.map((item) => {
...
function movementTable(direction) {
  ...
  return `<div class="table-wrap" data-layout-area="data-table"><table><thead><tr><th>Date</th><th>Item</th><th>Quantity</th><th>Reference</th></tr></thead><tbody>${rows.map((row) => {
